<?php
    ...
    $queryPeserta = mysqli_query($koneksi, "SELECT jurusan.nama_jurusan, pendaftran.* FROM pendaftran LEFT JOIN jurusan ON jurusan.id = pendaftran.id_jurusan WHERE deleted = 0 ORDER BY pendaftran.id DESC");
if (isset($_GET['delete'])){
    $id = $_GET ['delete'];

    $delete = mysqli_query($koneksi, ("UPDATE pendaftran SET deleted = 1 WHERE id = '$id'"));
}
// 3,2,1,0 (3:lolos sortir, 2:lolos wawancara, 1:lolos full, 0:tidak lolos) 
function customStatus($status){
    if($status == 1){
        $pesan = "Peserta Lolos Seleksi";
    } elseif ($status == 2){
        $pesan = "Lolos Wawancara";
    } elseif ($status == 3){
        $pesan = "Lolos Administrasi";
    } else {
...
if(isset($_POST['ubah_status'])){
    ...
    // ubah peserta ke kolom status dimana id sama dengan nilai post id
    $ubahStatus = mysqli_query($koneksi, "UPDATE pendaftran SET status='$status' WHERE id='$id'");
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a onclick="return confirm('Apakah anda yakin menghapus data ini??')" href="peserta.php?delete=<?php echo $dataPeserta ['id'] ?>" class="btn btn-danger btn-sm">Hapus</a>
                                    </td>
                                </tr>
